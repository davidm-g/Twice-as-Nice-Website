<?php
    ...
    if ($username !== $seller) { // If the user is the buyer
        header("Location: checkout.php?item_id=".urlencode($itemId)."&price=".urlencode($price)."&user=".urlencode($otherUser));
    } else {
        header('Location: /messages.php?user=' . urlencode($otherUser) . '&item=' . urlencode($itemId));
    }
    exit;
